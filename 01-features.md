...
- **Product-id override UI: dedicated admin page (capability-gated).** Moodle 5.2's `\core_payment\gateway` exposes no per-payable-item form hook, so the `digistore24_product_id` override does NOT live on course / activity edit forms. Instead it lives on a plugin admin page "Site administration → Plugins → Payment gateways → Digistore24 → Product mapping", backed by a dedicated table `paygw_digistore24_itemmap` keyed by `(component, paymentarea, itemid)`. Access is gated by a Moodle capability `paygw/digistore24:managemapping` (default assigned to Site administrator and Manager; admins can grant it to additional roles via the standard role permissions UI).
- **Refund / reversal bridge.** `core_payment` in Moodle 5.2 has no generic reversal API. The plugin therefore brings its own bridge: on a Digistore24 refund / chargeback IPN it (a) marks the payment reversed in its own bookkeeping table, (b) for `component = 'enrol_fee'` calls `enrol_fee` directly to unenrol the user, (c) for any other component fires a `paygw_digistore24\event\payment_reversed` event so custom plugins / observers can act, and (d) surfaces every reversal in the plugin admin report so an admin can always intervene manually.
